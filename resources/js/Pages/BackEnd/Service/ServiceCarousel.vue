...
const submit = () => {
    ...
    if (editId.value) {
        form.transform((data) => ({
            ...data,
            _method: "put",
        })).post(route("service-carousels.update", editId.value), {
            forceFormData: true,
            onSuccess: () => resetForm(),
        });
    } else {
        form.post(route("service-carousels.store"), {
            forceFormData: true,
            onSuccess: () => resetForm(),
        });
    }
...
const deleteData = (id) => {
    if (confirm("Delete?")) {
        form.delete(route("service-carousels.destroy", id));
    }
};
...
<template>
    <Head title="Service Carousel Management" />
    ...
    <Layout>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="border p-4 rounded shadow border-slate-200 h-fit">
                <h2 class="font-bold mb-3">Service Carousel List</h2>
